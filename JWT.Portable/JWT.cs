        ...
        static JsonWebToken()
        {
            HashAlgorithms = new Dictionary<JwtHashAlgorithm, Func<byte[], byte[], byte[]>>
            {
                { JwtHashAlgorithm.HS256, (key, value) => {
                        var sha = PCLCrypto.WinRTCrypto.MacAlgorithmProvider.OpenAlgorithm(MacAlgorithm.HmacSha256);
                        var hmacKey = sha.CreateKey(key);
                        return PCLCrypto.WinRTCrypto.CryptographicEngine.Sign(hmacKey, value);
                    }
                },
                { JwtHashAlgorithm.HS384, (key, value) => {
                        var sha = PCLCrypto.WinRTCrypto.MacAlgorithmProvider.OpenAlgorithm(MacAlgorithm.HmacSha384);
                        var hmacKey = sha.CreateKey(key);
                        return PCLCrypto.WinRTCrypto.CryptographicEngine.Sign(hmacKey, value);
                    }
                },
                { JwtHashAlgorithm.HS512, (key, value) => {
                        var sha = PCLCrypto.WinRTCrypto.MacAlgorithmProvider.OpenAlgorithm(MacAlgorithm.HmacSha512);
                        var hmacKey = sha.CreateKey(key);
                        return PCLCrypto.WinRTCrypto.CryptographicEngine.Sign(hmacKey, value);
                    }
                }
