...
namespace JWT
{
    public static class UnixEpoch
    {
        ...
        public static double GetSecondsSince(DateTime time) => Math.Round((time - Value).TotalSeconds);

        public static string GetSecondsSinceAsString(DateTime time) => GetSecondsSince(time).ToString(CultureInfo.InvariantCulture);
    }
}